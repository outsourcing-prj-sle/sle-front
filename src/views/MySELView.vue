<template>
  <div class="flex flex-col bg-white">
    <section class="flex gap-5 justify-between px-16 py-6 w-full bg-blue-200 text-neutral-700 max-md:flex-wrap max-md:px-5 max-md:max-w-full">
      <div class="flex flex-col my-auto">
        <h2 class="text-xl font-bold text-left">나의 SEL 알기</h2>
        <p class="mt-4 text-base font-medium text-left">
          활동 참여를 통해 나의 SEL을 자세히 알아볼 수 있습니다.<br />기간 내 맞춰서 참여할 수 있도록 해주세요.
        </p>
      </div>
      <img src="@/assets/img/logo_sel.png" alt="Logo" class="h-[150px] aspect-[2.5]" />
    </section>
    <section class="flex flex-col px-20 mt-7 w-full text-base max-md:px-5 max-md:max-w-full">
        ...
        >
          ...
          <tbody class="">
            ...
            >
              ...
              <td class="my-auto text-zinc-800 px-2 w-[260px] text-center">{{ info.date }}</td>
              <td class="my-auto text-zinc-800 px-2 w-[200px] text-center">
                ...
                <div
                  v-if="info.state === 'closed'"
                >
                  미참여-마감
                </div>
                <div
                  v-if="info.state === 'start'"
                >
                  <button class="justify-center self-stretch px-4 py-1 font-medium text-white whitespace-nowrap bg-blue-400 rounded-[999px]">
                    시작하기
                  </button>
                </div>
                <div
                  v-if="info.state === 'restart'"
                >
                  <button class="justify-center self-stretch px-4 py-1 font-medium text-white bg-blue-600 rounded-[999px]">
                    이어서 진행하기
                  </button>
                </div>
              </td>
              <td class="my-auto text-zinc-800 px-2 w-[130px] text-center">
                <div v-if="info.state === 'fin'">
                  <button class="justify-center self-stretch px-4 py-1 font-medium text-white bg-slate-400 rounded-[999px]">
                    내 답안 보기
                  </button>
                </div>
                <div v-else>
